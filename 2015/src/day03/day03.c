...
static uint8_t pos_move(pos* p, char d) {
    switch (d) {
    case '^':
        ++p->y;
        break;
...
static bool pos_equal(const pos* a, const pos* b) {
    return a->x == b->x && a->y == b->y;
}

static uint8_t visited_add(pos** v, uint16_t* l, const pos* n) {
...
uint16_t day03_part1(const char* input) {
    ...
    for (const char* d = s->dirs; *d != '\0'; ++d) {
        if (pos_move(&s->curr, *d) != 0 || visited_add(&s->visited, &s->visited_len, &s->curr) != 0) {
            goto defer;
        }
    }
    r = s->visited_len;
...
uint16_t day03_part2(const char* input) {
    ...
    for (uint16_t i = 0; s->dirs[i] != '\0'; ++i) {
        ...
        if (pos_move(curr_adj, s->dirs[i]) != 0 || visited_add(&s->visited, &s->visited_len, curr_adj) != 0) {
            goto defer;
        }
    }
    r = s->visited_len;
